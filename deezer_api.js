var DZ = {
    ...
    onDeezerLoaded: function (settings) {
        if (typeof (settings.user) != 'undefined' && settings.user !== null) {
    ...
    inapploaded: function (data) {
        ...
        if (!playercontrol.player_loaded) {
        ...
        var get_user_id = typeof (data.user_id) == 'boolean' && data.user_id;
        var player_options = DZ.player_controler.onPlayerLoaded();
        var load_options = {
            player: player_options,
            framework: {
                text: {
                    add: trad.music.favorite_add_to_my_music,
                    remove: trad.music.favorite_remove_to_my_music,
                    add_playlist: trad.music.favorite_add_to_my_music,
                    buy: trad.music.buy,
                    share: trad.music.naboo_share_link,
                    follow: trad.user.label_follow,
                    unfollow: trad.user.label_unfollow
                }
            },
            user: (get_user_id ? {
                id: USER.USER_ID
        ...
        if (false && DZ.inapploadedCount == 1) {
            var addressValue = $.address.value();
            var base_app_path = app.base_page;
            if (addressValue == '/' + base_app_path) {
                $.address.value(base_app_path + '/');
            } else if (addressValue == '/' + base_app_path + "/") {
                $.address.value(base_app_path);
            }
        }
        DZ.inapploadedCount++;
...
DZ.SIMPLE_API = {
    ...
    apiCall: function (path, request_method, args, callback) {
        try {
            ...
            if (request_method != 'get') path_args.push("request_method=" + request_method);
            if (args !== null) {
            ...
            if (DZ.token !== null) path_args.push("access_token=" + DZ.token);
...
DZ.Event = {
    ...
    framework: {
        'follow_plugin.onDeleteFavorite': 'plugin_on_unfollow',
        'follow_plugin.onAddFavorite': 'plugin_on_follow'
    },
    callbacks: {
        'music.end': [],
        'music.start': [],
    ...
    subscribe: function (evt, callback, once) {
        try {
            if (!DZ.initialized && evt != DZ.Event.SDK_READY) throw DZ.Exception('init');
            if (typeof (callback) == 'object') return DZ.Event.subscribeList(evt, callback, once);
            ...
            case DZ.Event.MUSIC_START:
                ...
                break;
            default:
                if (typeof (DZ.Event.callbacks[evt]) == 'undefined') DZ.Event.callbacks[evt] = [];
                DZ.Event.callbacks[evt].push(callback);
                break;
            }
        } catch (e) {
    ...
    unsubscribe: function (evt) {
        try {
            if (!DZ.initialized) throw DZ.Exception('init');
            if (!evt || typeof (DZ.Event.callbacks[evt]) == 'undefined') return false;
    ...
    triggerEvent: function (response) {
        try {
            ...
            if (response.evt && DZ.Event.callbacks[response.evt]) {
                ...
                for (i = 0; i < DZ.Event.callbacks[response.evt].length; i++) {
...
DZ.log = function () {
    if (DZ.DEBUG && typeof (console) != 'undefined' && typeof (console.log) != 'undefined') {}
};
...
DZ.deezer = {
    ...
    subscribeEvents: function () {
        Events.addEvent('DZ', Events.user.addFavorite, function (evt, params) {
            ...
            if (typeof (params[id_names[params.type]]) == 'undefined') {
                return false;
            }
            params.element_id = params[id_names[params.type]];
    ...
    addToPlaylist: function (arg) {
        ...
        api.call('song_getListAllData', {
            sng_ids: song_ids,
            user_id: USER.USER_ID
        }, function (request, position) {
            if (request.error.length === 0) {
                musicbox.open(null, request.results.data, {
                    position: 'mouse',
                    x: position.x,
                    y: position.y
                });
    ...
    follow: function (favorite_element) {
        ...
        api.call('user_getData', {
            user_id: id,
            array_default: ['USER_ID', 'DISPLAY_NAME', 'USER_PICTURE']
        }, function (request, callback_parameters) {
            if (request.error.length === 0) {
                favorite.add({
    ...
    unfollow: function (favorite_element) {
        ...
        api.call('user_getData', {
            ...
            if (request.error.length === 0) {
                favorite.remove({
                    id: callback_parameters.id,
                    USER_ID: callback_parameters.id,
                    type: 'friend'
                });
            }
        }, {
    ...
    removeFavorite: function (favorite_element) {
        favorite_element.PLAYLIST_ID = favorite_element.id;
        favorite_element.ALB_ID = favorite_element.id;
        favorite_element.RADIO_ID = favorite_element.id;
        favorite_element.ART_ID = favorite_element.id;
    ...
    askFavorites: function (send_queue) {
        ...
        console.log("CALL BACK VALUE", callback_values);
    ...
    ui: {
        register: function (options) {
            ...
            modal.open('/register.php?redirect_type=page&redirect_link=' + encodeURIComponent(redirect_link) + '&app_id=' + app.app_id);
            return true;
        },
        ...
        appRequest: function (options) {
            ...
            var id_facebox = loadFacebox('lightbox/apprequest.php?to=' + options.to.join(',') + "&app_id=" + DZ.app_id + "&message=" + encodeURIComponent(message));
            DZ.deezer.ui.appRequest_opened = true;
            Events.subscribeOnce(Events.lightbox.close + '_' + id_facebox, function (evt, args) {
...
DZ.communication = {
        ...
        try {
            ...
            if (hashSplitted.length > 0) {
                ...
                if (type == "channel") {
                    ...
                    window.opener.location.href = window.opener.location.href + "&autoplay=true";
                    window.close();
                }
            }
            return return_value;
    ...
    proxy_send: function (message, domain) {
        try {
            ...
            var channel_message = DZ.JSON.stringify(message) + "";
    ...
    send: function (framePath, method, args, domain, plugin) {
        try {
            ...
            if (DZ.communication.postMessage) {
                ...
                var post_domain = domain == 'app' ? DZ.SETTING_HOST_APP : DZ.SETTING_HOST_SITE;
                iframeToSend.postMessage(DZ.JSON.stringify(message), '*');
            } else {
    ...
    receive: function (evt) {
        try {
            ...
            if (typeof (data.method) == 'undefined') return false;
...
DZ.player = {
        ...
        try {
            ...
            if (typeof (DZ.Event.player[event] != 'undefined')) {
                if (event == 'TRACKS_LOADED') {
                    ...
                    DZ.player._core.changeOrder(value);
                    DZ.Event.triggerEvent({
    ...
    loadPlayer: function (options) {
        try {
            ...
            if (options.drawPlayer) {
                if (typeof (options.cover) != 'undefined') arg.push("cover=" + (options.cover ? 'true' : 'false'));
                if (typeof (options.playlist) != 'undefined') arg.push("playlist=" + (options.playlist ? 'true' : 'false'));
                if (typeof (options.width) != 'undefined') arg.push("width=" + options.width);
                if (typeof (options.height) != 'undefined') arg.push("height=" + options.height);
                if (typeof (options.format) != 'undefined') arg.push("format=" + options.format);
            ...
            src = src + arg.join('&');
            DZ.player.player_iframe.onload = function () {};
    ...
    loadWidget: function (options) {
        try {
            DZ.player.attachEvents();
            DZ.player.player_type = 'widget';
            options = options || {};
            if (typeof (options.containerId) == 'undefined') {
                throw "You must define a containerId (options.containerId) !";
            }
            var container = document.getElementById(options.containerId);
            if (typeof (container) == 'undefined' || container === null) {
                throw "The container you defined doesn't exists !";
            }
            ...
            var scover = 'true';
            var autoplay = 'false';
            if (typeof (options.playlist) != 'undefined') playlist = options.playlist ? 'true' : 'false';
            if (typeof (options.cover) != 'undefined') cover = options.cover ? 'true' : 'false';
            if (typeof (options.scover) != 'undefined') scover = options.scover ? 'true' : 'false';
            if (typeof (options.autoplay) != 'undefined') autoplay = options.autoplay ? 'true' : 'false';
            var src = DZ.SETTING_HOST_SITE + '/fr/plugins/player?' + 'autoplay=' + autoplay + '&playlist=' + playlist + '&width=' + width + '&height=' + height + '&scover=' + autoplay + '&title=';
            src += "&type=songList";
            src += "&songIds=" + options.songIds.join(',');
            src += "&dzwidget=true";
            src += "&channel=" + DZ.channelUrl;
            DZ.player.player_iframe = document.createElement('iframe');
            DZ.player.player_iframe.id = "dzplayer";
            DZ.player.player_iframe.src = src;
            DZ.player.player_iframe.style.width = width + "px";
            DZ.player.player_iframe.style.height = height + "px";
            DZ.player.player_iframe.style.border = "none";
            container.appendChild(DZ.player.player_iframe);
        } catch (e) {
            DZ.catchException(e);
            return false;
        }
    ...
    get_play_fct_args: function (args) {
        try {
            switch (args.length) {
            case 1:
                break;
            case 2:
                ...
                break;
            case 3:
                if (typeof (arguments[1]) == 'number') index = arguments[1];
    ...
    getPlayArguments: function (args) {
        try {
            ...
            for (var i = 1; i < args.length; i++) {
                ...
                switch (type) {
                case 'function':
                    if (types_index['function'] === 0) {
                        return_args.callback = args[i];
                    }
                    types_index['function']++;
                    break;
                case 'number':
    ...
    playTracks: function () {
        try {
            ...
            var play_arguments = DZ.player.getPlayArguments(arguments);
            play_arguments.tracks = arguments[0];
            var trackList = play_arguments.tracks.join('|');
            DZ.Event.subscribe(DZ.Event.player.TRACKS_LOADED + "_playTracks_" + trackList, [
                function (data) {
                    play_arguments.callback(data);
                }
            ], true);
            DZ.player._core.setType("playTracks", trackList);
            DZ.communication.send("frames.dzplayer", "DZ.player_controler.playTracks", {
                trackList: trackList,
                index: play_arguments.index,
                autoplay: play_arguments.autoplay,
                offset: play_arguments.offset
            }, 'deezer');
        } catch (e) {
            DZ.catchException(e);
        }
    ...
    playPlaylist: function () {
        try {
            ...
            var play_arguments = DZ.player.getPlayArguments(arguments);
            play_arguments.playlist_id = arguments[0];
            var playlist_id = play_arguments.playlist_id;
            DZ.Event.subscribe(DZ.Event.player.TRACKS_LOADED + "_playPlaylist_" + playlist_id, [
                function (data) {
                    play_arguments.callback(data);
                }
            ], true);
            DZ.player._core.setType("playPlaylist", playlist_id);
    ...
    playAlbum: function () {
        try {
            ...
            var album_id = arguments[0];
            var play_arguments = DZ.player.getPlayArguments(arguments);
            DZ.Event.subscribe(DZ.Event.player.TRACKS_LOADED + "_playAlbum_" + album_id, [
                function (data) {
                    play_arguments.callback(data);
                }
            ], true);
            DZ.player._core.setType("playAlbum", album_id);
    ...
    playRadio: function () {
        try {
            ...
            var radio_id = arguments[0];
            var play_arguments = DZ.player.getPlayArguments(arguments);
            DZ.Event.subscribe(DZ.Event.player.TRACKS_LOADED + "_playRadio_" + radio_id, [
                function (data) {
                    play_arguments.callback(data);
                }
            ], true);
            DZ.player._core.setType("playRadio", radio_id);
    ...
    playSmartRadio: function () {
        try {
            ...
            var radio_id = arguments[0];
            var play_arguments = DZ.player.getPlayArguments(arguments);
            DZ.Event.subscribe(DZ.Event.player.TRACKS_LOADED + "_playSmartRadio_" + radio_id, [
                function (data) {
                    play_arguments.callback(data);
                }
            ], true);
            DZ.player._core.setType("playSmartRadio", radio_id);
            DZ.communication.send("frames.dzplayer", "DZ.player_controler.playSmartRadio", {
                radio_id: radio_id,
                index: 0,
                autoplay: play_arguments.autoplay
            }, 'deezer');
    ...
    playExternalTracks: function () {
        try {
            ...
            var play_arguments = DZ.player.getPlayArguments(arguments);
            var trackList_join = trackList.join('|');
    ...
    setTrackList: function (songList, callback) {
        try {
            if (!DZ.player.loaded) {
                throw DZ.Exception('PLAYER_NOT_LOADED');
            }
            callback = callback || function () {};
            DZ.Event.subscribe(DZ.Event.player.TRACKS_LOADED, callback, true);
            DZ.communication.send("frames.dzplayer", "DZ.player_controler.setSongList", songList, 'deezer');
        } catch (e) {
            DZ.catchException(e);
            return false;
        }
    },
    setBlindTestMode: function (activ, options) {
    ...
    _core: {
        ...
        on_volume_changed: function (value, evt) {
            DZ.player._core.volume = value;
        },
        on_shuffle_changed: function (value, evt) {
            DZ.player._core.shuffle = value;
        },
        on_repeat_changed: function (value, evt) {
            DZ.player._core.repeat = value;
        },
        on_mute_changed: function (value, evt) {
        ...
        onLoadedInPlayer: function (data) {
            ...
            for (i = 0; i < DZ.player._core.tracks.length; i++) {
        ...
        getTrackList: function () {
            if (!DZ.player._core.isRunning()) return [];
...
DZ.player_controler = {
    abstract_methods: ['onPlayerLoaded', 'onLogin', 'onLogout', 'subscribeEvents', 'doAction', 'playTracks', 'playAlbum', 'playRadio', 'playSmartRadio', 'setSongList'],
    parent: {
    ...
    init: function (options) {
        try {
            DZ.player_controler.subscribeEvents();
            options.widgetOptions.drawPlayer = typeof (options.widgetOptions.emptyPlayer) == 'undefined';
            widget.init(options);
            return true;
        } catch (e) {
            DZ.catchException(e);
    ...
    doAction: function (args) {
        try {
            ...
            if (command == 'repeat') {
                value = Math.max(0, Math.min(2, value * 1));
                if (value != dzPlayer.getRepeat()) {
                    playercontrol.doAction('repeat', value);
                }
                return true;
            }
            if (command == 'changeTrackOrder') {
                playercontrol.changeTrackOrder(value);
                return true;
            }
    ...
    subscribeEvents: function () {
        try {
            ...
            Events.addEvent('DZ', Events.player.displayCurrentSong, function (evt, val) {
                ...
                DZ.communication.send("parent", "DZ.player.receiveEvent", {
                    evt: 'CURRENT_TRACK',
                    val: dzPlayer.getCurrentSongInfo().SNG_ID
                });
            });
            Events.addEvent('DZ', 'player_track_loaded', function (evt, data) {
                if (typeof (data.tracks) != 'undefined') {
                    ...
                    var tracks = [];
                    for (i = 0; i < widgetLines.length; i++) {
    ...
    preloadTrackEnd: function (status, sng_id) {
        try {
            console.log("PRELOAD END ", sng_id, status);
    ...
    preloadTrack: function (sng_id) {
        ...
        api.call('song_getListData', {
            ...
            var trackDatas = request.results.data;
            if (trackDatas.length > 0) {
                dzPlayer.trigger("audioPlayer_preloadTrack", [
                    [trackDatas[0]]
                ]);
            } else {
                DZ.player_controler.preloadTrackEnd('unknown');
    ...
    filterTracks: function (tracks) {
        ...
        for (i = 0; i < tracks.length; i++) {
            if (tracks[i].TYPE == 'JINGLE') tracks[i].SNG_ID = 'JINGLE_' + i;
...
DZ.player_controler.playRadio = function (arg) {
    try {
        widget.loadRadio('radio-' + arg.radio_id, function (tracks) {
            Events.trigger("player_track_loaded", {
                tracks: tracks,
                type: 'playRadio',
                id: arg.radio_id
...
DZ.player_controler.playSmartRadio = function (arg) {
    try {
        widget.loadRadio('artist-' + arg.radio_id, function (tracks) {
            Events.trigger("player_track_loaded", {
                tracks: tracks,
                type: 'playSmartRadio',
                id: arg.radio_id
...
DZ.player_controler.setSongList = function (songList) {
    try {
        player_light.loadSongs(songList);
    } catch (e) {
        DZ.catchException(e);
    }
};;
...
DZ.util = {
    ...
    uniqid: function (length) {
        try {
            ...
            length = Math.max(7, Math.floor(length));
            var length2 = Math.min(7, Math.floor(length / 2));
            var length1 = length - length2;
            var formatSeed = function (seed, reqWidth) {
                seed = parseInt(seed, 10).toString(16);
                if (reqWidth < seed.length) {
                    return seed.slice(seed.length - reqWidth);
...
DZ.framework = {
    classes: ['dz-library', 'dz-addtoplaylist', 'dz-share', 'dz-buy', 'dz-link', 'dz-follow'],
    text: {
        add: '',
        remove: '',
        follow: '',
    ...
    dispatchReconnect: function () {},
    onLoad: function (settings) {
        DZ.framework.text = settings.text;
        DZ.framework.parseWaiting();
    },
    waitingParse: [],
    parseWaiting_running: false,
    parseWaiting: function () {
        DZ.framework.parseWaiting_running = true;
        for (var i = 0; i < DZ.framework.waitingParse.length; i++) {
            DZ.framework.parse(DZ.framework.waitingParse[i], true);
        }
        DZ.framework.waitingParse = [];
        DZ.framework.askQueue();
        DZ.framework.parseWaiting_running = false;
    },
    forbidParse: true,
    parse: function (selector, force) {
        ...
        if (DZ.framework.forbidParse && !force) {
            DZ.framework.waitingParse.push(selector);
            return false;
        }
        var $container = DZ.query(selector);
        if ($container.length > 0) {
            for (var i = 0; i < DZ.framework.classes.length; i++) {
                $container.find('.' + DZ.framework.classes[i]).each(function () {
                    var $this = DZ.query(this);
                    if (!$this.hasClass('dz-parsed')) {
                        DZ.framework.parsefunctions[DZ.framework.classes[i]]($this);
                        $this.addClass('dz-parsed dz-inapp dz-widget');
                    }
                });
            }
        }
        if (!DZ.framework.parseWaiting_running) {
            DZ.framework.askQueue();
        }
    },
    asking_queue: {
    ...
    addFavoriteElement: function (type, id, $el) {
        if (typeof (DZ.framework.favoriteElements[type]) == 'undefined') {
            return false;
        }
        id = 'id_' + id;
        if (typeof (DZ.framework.favoriteElements[type][id]) == 'undefined') {
            DZ.framework.favoriteElements[type][id] = [];
        }
        DZ.framework.favoriteElements[type][id].push($el);
        if (typeof (DZ.framework.asking_queue[type]) != 'undefined') {
            DZ.framework.asking_queue[type][id] = true;
        }
    },
    onFavoriteTriggers: {
        'friend': {
            event_add: 'follow_plugin.onAddFavorite',
            event_remove: 'follow_plugin.onDeleteFavorite',
    ...
    dispatchFavoriteState: function (type, id, value) {
        ...
        for (var i = 0; i < DZ.framework.favoriteElements[type][id].length; i++) {
            DZ.framework.changeButtonState(DZ.framework.favoriteElements[type][id][i], value, type);
        }
    },
    changeButtonState: function ($el, is_favorite, type) {
        var textRemove = DZ.framework.text.remove;
        var textAdd = DZ.framework.text.add;
        var actionAdd = 'add';
        var actionRemove = 'remove';
        if (type == 'friend') {
            textRemove = DZ.framework.text.unfollow;
            textAdd = DZ.framework.text.follow;
            actionAdd = 'follow';
    ...
    askQueue: function () {
        DZ.framework.forbidParse = true;
        var send_queue = {};
        var send_request = false;
        for (var type in DZ.framework.asking_queue) {
            send_queue[type] = [];
            for (var id in DZ.framework.asking_queue[type]) {
        ...
        if (send_request) {
            ...
            DZ.framework.forbidParse = false;
        }
    },
    callbackQueue: function (callback_values) {
        for (var type in callback_values) {
            for (var i = 0; i < callback_values[type].length; i++) {
                DZ.framework.dispatchFavoriteState(type, callback_values[type][i].id, callback_values[type][i].value);
            }
        }
        DZ.framework.forbidParse = false;
        DZ.framework.parseWaiting();
...
DZ.navigation = {
    ...
    goTo: function (deezer_page) {
        ...
        DZ.communication.callDeezerMethod('DZ.deezer.loadbox', deezer_page.substr(22));
    },
        ...
        Deezer_object: {
            ...
            log: function () {
                ...
                for (var i = 0; i < arguments.length; i++) {
                    var arg = typeof (arguments[i]) == 'string' ? arguments[i] : DZ.JSON.stringify(arguments[i]);
                    log.push(arg);
                }
        ...
        callDeezer: function (json) {
            try {
                ...
                if (json.method.substr(0, 20) == "DZ.player_controler.") {
                    ...
                    args = DZ.JSON.stringify(json.args);
    ...
    android: {
        ...
        callDeezer: function (json) {
            try {
                var args;
                if (json.method == 'DZ.inapploaded') {
                    Deezer.webviewLoaded(DZ.JSON.stringify({
        ...
        wrapCall: {
            ...
            askFavorites: function (send_queue) {
                Deezer.askFavorites(DZ.JSON.stringify(send_queue));
                return true;
            }
        }
    }
...
function dzloader() {
    ...
    if (!dzchannel) {
        ...
        DZ.query(document).ready(function () {
            ...
            DZ.init_framework();
            if (whereami.context == DZ.CONTEXT.PLAYER && whereami.side == DZ.CONTEXT.APP) {
                DZ.css.init();
            }
        });
